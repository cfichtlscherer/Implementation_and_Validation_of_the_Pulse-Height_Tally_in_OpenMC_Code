...
void PrimaryGeneratorAction::GeneratePrimaries(G4Event* anEvent)
{
  ...
  //
  G4double r = 2.8*G4UniformRand(), phi = twopi*G4UniformRand();    //CPF
  G4double px = std::sqrt(r)*std::cos(phi)*cm,                      //CPF
           py = std::sqrt(r)*std::sin(phi)*cm,                      //CPF
           pz = 0.*cm;                                             //CPF

  fParticleGun->SetParticlePosition(G4ThreeVector(px, py, pz));     //CPF
