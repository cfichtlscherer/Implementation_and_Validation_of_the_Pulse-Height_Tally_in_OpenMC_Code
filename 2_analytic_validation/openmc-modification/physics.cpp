...
void sample_photon_reaction(Particle& p)
{
  ...
  double prob = prn(p.current_seed());
  ...
  if (prob >= 0.5) {
    if (p.E() >= 1000000){
       double u[3] = {0, 1, 0};
       p.create_secondary(p.wgt(), u, 0.25 * p.E(), ParticleType::photon);
       p.E() = 0.25 * p.E();
       }
    else{
      p.create_secondary(p.wgt(), p.u(), 0.25 * p.E(), ParticleType::photon);
      p.E() = 0.25 * p.E();
    }
  }
}
