...
def extract_energy_bins_mcnp(mcnp_outputfile):
    """ this function extracts the pht output of a mcnp calculation"""
...
def evaluate_run(run_name):
    ...
    viewed_range = range(301)

    plt.plot(viewed_range, [0] + list(np.load("ks_openmc_mcnp_2048.npy")), color=colors[2], label="OpenMC / MCNP6")
    plt.plot(viewed_range, [0] + list(np.load("ks_openmc_geant_2048.npy")), color=colors[3], label="OpenMC / Geant4")
    plt.plot(viewed_range, [0] + list(np.load("ks_mcnp_geant_2048.npy")), color=colors[4], label="Geant4 / MCNP6")

    plt.plot(viewed_range, 301 * [0.042], color = colors[1], ls="--", marker="")
    plt.text(260, 0.03, "$d_{0.05} = 0.042$")
    ...
    plt.plot(66, np.load("ks_openmc_mcnp_2048.npy")[65], marker="x", color="black")
    plt.plot(66, np.load("ks_openmc_geant_2048.npy")[65], marker="x", color="black")
    plt.plot(66, np.load("ks_mcnp_geant_2048.npy")[65], marker="x", color="black")

    plt.text(50, 0.13, "Cs-137 spectrum")
    plt.arrow(66, 0.126, 0, -0.01, head_width=2, head_length=0.005, color="black")
    plt.savefig(figures_path + 'openmc_compare_all_spectra.pgf', bbox_inches='tight')


def generate_arbitrary_spectra_plot(mcnp_npy, openmc_npy, run_name, energy):

    mcnp_spectrum = mcnp_npy[4:]
    openmc_spectrum = openmc_npy[3:]
    ...
    for ax in axs:
        ax.label_outer()
    plt.savefig(run_name + "/example_spectrum_plot-" + energy + ".pdf")


def check_bins():
    mcnp_bins = extract_energy_bins_mcnp("run-31/mcnp_input_285.o")

    parameter_dic = {"energy": 2.84 * 1e6}
    ...
    print("Number".ljust(20) + "OpenMC bin".ljust(20) + "MCNP bin".ljust(20) + "Difference")

    for index, bin_value in enumerate(parameter_dic["bins"]):
        print(str(index).ljust(20) + str(bin_value).ljust(20) + str(mcnp_bins[index]*1e6).ljust(20) + str(bin_value - mcnp_bins[index]*1e6))

#for i in [2.41, 2.68, 2.84]:
#    parameter_dic = {"energy": i * 1e6}
#    bins_middle = np.linspace(parameter_dic["energy"] / 2048, parameter_dic["energy"], 2048)
#    parameter_dic["bins"] = [0, 0.001, 1] + list(bins_middle + 0.5 * (bins_middle[2] - bins_middle[1])) + [20e6]
#    print(i, parameter_dic["bins"][393 -2], parameter_dic["bins"][371-2], parameter_dic["bins"][437-2])


#check_bins()

#for i in [145, 169, 193, 241, 268, 284]:
#    mcnp_bins = extract_energy_bins_mcnp("run-31/mcnp_input_" + str(i) + ".o")
#    np.save("run-32/bins-" + str(i), mcnp_bins)

# for i in [145, 169, 193, 241, 268, 284]:
#    generate_arbitrary_spectra_plot(np.load("run-31/mcnp_spectrum_" + str(i) + ".npy"), np.load("run-39/openmc_results_" + str(i) + ".npy"), "run-39", str(i))

'''
print("KS-test 200 kev".ljust(50), ks_test(np.load("run-26/mcnp_spectrum_200.npy")[4:-1], np.load("big_run-6-openmc/openmc_results_200.npy")[3:-1]))
print("KS-test 250 kev".ljust(50), ks_test(np.load("run-26/mcnp_spectrum_250.npy")[4:-1], np.load("big_run-6-openmc/openmc_results_250.npy")[3:-1]))
print("KS-test 300 kev".ljust(50), ks_test(np.load("run-26/mcnp_spectrum_300.npy")[4:-1], np.load("big_run-6-openmc/openmc_results_300.npy")[3:-1]))
'''
