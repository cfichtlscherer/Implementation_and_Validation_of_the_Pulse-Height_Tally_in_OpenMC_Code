...
def start_run(parameter_dic):
    ...
    files_here = os.listdir()
    runs = [f for f in files_here if "run-" in f]
    new_folder = "run-" + str(len(runs) + 1 - 4)
...
def start_big_run(parameter_dic):
    ...
    for energy in [i for i in range(1,301)] + [66.17]:
        parameter_dic["energy"] = energy * 1e4
        parameter_dic["bins"] = [0, 0.001, 1] + list(np.linspace(parameter_dic["energy"] / 2048, parameter_dic["energy"], 2048)) + [20e6]
        build_mcnp_input(parameter_dic)
...
def start_big_openmc_run(parameter_dic, folder_name):
    ...
    files_here = os.listdir(folder_name)
    generate_logfile(parameter_dic)
    os.system("mv logfile " + folder_name)
    
    for energy in [66.17] + list(range(1,301)):                                           
        print(energy)   
...
print("103:105")
print(parameter_dic["bins"][103:105])
print("1860:1864")
print(parameter_dic["bins"][1860:1864])
print("1871:1875")
